...
def run_simulation(base_dir=get_climate_data_path(), days_ahead=6):
    ...
    conn  = sqlite3.connect(_get_db_path()); cur = conn.cursor()
    cur.execute(
        "SELECT date, day, month, year, tmin, tmax, prcp, eto "
        "FROM weather WHERE date BETWEEN ? AND ? ORDER BY date",
    ...
    if rows:
        collected = [{
            ...
            'day':   r[1], 'month': r[2], 'year': r[3],
            'tmin':  r[4], 'tmax': r[5], 'prcp': r[6], 'eto': r[7]
        } for r in rows]
        print(f"Found {len(collected)} days in DB")
    else:
        print("No DB data — falling back to flat-files")
        collected = []
        files = [f for f in os.listdir(base_dir) if f.startswith('weather_') and f.endswith('.txt')]
        files.sort(key=lambda fn: datetime.strptime(fn[8:-4], '%Y-%m-%d').date())
        for fn in files:
                ...
                for line in open(path).readlines()[1:]:
                    ...
                    if today <= date <= end:
                        collected.append({
                            ...
                            'tmin': float(parts[3]), 'tmax': float(parts[4]),
                            'prcp': float(parts[5]), 'eto': float(parts[6])
                        })
                        print(f"Found {date} in {fn}")
            except Exception as e:
    ...
    # Write temp file
    temp = os.path.join(base_dir, "temp_aquacrop_weather.txt")
    with open(temp, 'w') as f:
        f.write("Day\tMonth\tYear\tTmin(C)\tTmax(C)\tPrcp(mm)\tEt0(mm)\n")
        for i, d in enumerate(collected):
            line = f"{d['day']}\t{d['month']}\t{d['year']}\t{d['tmin']}\t{d['tmax']}\t{d['prcp']}\t{d['eto']}"
            if i < len(collected)-1:
                line += "\n"
            f.write(line)
    print(f"Created temp file with {len(collected)} entries")

    aquacrop_simulation(temp)
    os.remove(temp)
