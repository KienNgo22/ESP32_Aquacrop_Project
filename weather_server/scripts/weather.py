...
from db import _init_db, _upsert_day
...
def get_climate_data_path():
    return get_server_root() / "climate_data"
...
CLIMATE_DIR = get_climate_data_path()
CLIMATE_DIR.mkdir(parents=True, exist_ok=True)          # create folder if needed
db_file = CLIMATE_DIR / "weather.db"                    # now this works!
_init_db()                                              # create DB/table if missing
...
def write_forecast_data(forecast_data, base_dir=None):
    ...
    for day_dict in forecast_data:
        for date_str, w in day_dict.items():
            dt = datetime.strptime(date_str, "%Y-%m-%d")
            _upsert_day(
                date_str,
                dt.day, dt.month, dt.year,
                w["tmin"], w["tmax"],
                w["precipitation"], w["eto"]
            )
            print(f"[DEBUG] Upserted {date_str} to DB")
    # Show directory contents so you can confirm weather.db is present
    print("[DEBUG] climate_data contains:", [p.name for p in CLIMATE_DIR.iterdir()])
